...
public class Admin {
    ...
    @PersistenceContext(unitName = "carrental-unit", type = PersistenceContextType.EXTENDED)
    private EntityManager carRentalEntityManager;

    @PersistenceContext(unitName = "bank-unit", type = PersistenceContextType.EXTENDED)
    private EntityManager bankEntityManager;
    ...
    public void updateData() {
        ...
        Bank bank = new Bank("123", "Mort Yao", 800);
        bankEntityManager.persist(bank);
    ...
    private void cleanUser() {
        List<User> list = userEntityManager.createQuery("SELECT u from User as u").getResultList();
        for (User user : list)
            userEntityManager.remove(user);
    }

    // Clean the CarRental table.
    private void cleanCarRental() {
        List<CarRental> list = carRentalEntityManager.createQuery("SELECT c from CarRental as c").getResultList();
        for (CarRental carRental : list)
            carRentalEntityManager.remove(carRental);
    }
